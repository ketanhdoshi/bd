...
import static com.kd.kdc.MySourceConnectorConfig.API_URL_CONFIG;
import static com.kd.kdc.MySourceConnectorConfig.TOPIC_CONFIG;
import static com.kd.kdc.MySourceConnectorConfig.SLEEP_CONFIG;
 ...
 */
  ...
  public List<Map<String, String>> taskConfigs(int maxTasks) {
    List<Map<String, String>> tconfigs = new ArrayList<>(maxTasks);
    for (int i = 0; i < maxTasks; i++) {
        Map<String, String> tconfig = new HashMap<>(3);
        tconfig.put(API_URL_CONFIG, config.getString(API_URL_CONFIG));
        tconfig.put(SLEEP_CONFIG, Integer.toString(config.getInt(SLEEP_CONFIG)));
        tconfig.put(TOPIC_CONFIG, config.getString(TOPIC_CONFIG));
